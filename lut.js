...
export class LUT {
    constructor(color_list) {
        ...
        if (color_list) {
            for (let i = 0; i < color_list.length; ++i) {
                this.addStop(i / (color_list.length - 1), color_list[i])
            }
        }
    ...
    drawDiscrete(ctx, n_steps) {
        // let ctx = this.canvas.getContext('2d');
        let start = 0;
        let end = 0;
        // let n = this.colors.length;
        let step = ctx.canvas.width / n_steps;
        // for (let stop of this.colors) {
        for (let i = 0; i < n_steps; i++) {
            ...
            ctx.fillStyle = color;
            ctx.fillRect(Math.floor(start), 0, Math.floor(end), ctx.canvas.height);
            start = end;
        }
    }

    drawCustom(ctx, n_steps, draw_fn) {
        // let ctx = this.canvas.getContext('2d');
        draw_fn(ctx, n_steps);
    }
}
